...
Next, download Sphinx (http://www.sphinxsearch.com/) . Get the latest
version in the 0.9.8 series. There may be compatibility issues with
the 0.9.9 series. Install by doing the standard
 ./configure && make && make install

And, finally, install some needed ruby packages by doing
 gem install chronic libxml-ruby rake rack thin packet fast_xs --no-ri --no-rdoc
 gem install mysql --no-ri --no-rdoc -- --with-mysql-config=/usr/bin/mysql_config
 gem install whenever --source http://gemcutter.org/ --no-ri --no-rdoc
...
+nginx.conf.sample+. Examine it, modify it (or don't), and stick it in
+/etc/ngnix.conf+ (on CentOS) or +/etc/nginx/nginx.conf+ (on
Debian). Then, on CentOS, start nginx and make sure it stays started:
