...
And, finally, install some needed ruby packages by doing
 ...
 gem install whenever thinking-sphinx ts-delayed-delta --source http://gemcutter.org/ --no-ri --no-rdoc
...
to manually start one or more services:
 cap my_site thin:start sphinx:start

FIXME: we need to figure out how to get the delayed jobs worker working.
